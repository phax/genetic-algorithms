...
/**
 * Continue up to a known optimum.
 *
 * @author Philip Helger
 */
public class ContinuationKnownOptimum extends AbstractContinuation
{
  private static final Logger LOGGER = LoggerFactory.getLogger (ContinuationKnownOptimum.class);
  ...
  protected boolean internalShouldContinue (@Nonnull final IPopulation aPopulation)
  {
    ...
    if (nCmp == 0)
    {
      if (LOGGER.isDebugEnabled ())
        LOGGER.debug ("Found optimum fitness of " + dKnownBestFitness + "!");
    }
    else
      if (nCmp > 0)
      {
        LOGGER.warn ("Found something better than the optimum: " +
                     dKnownBestFitness +
                     " vs " +
                     m_dOptimumFitness +
                     "!!!");
