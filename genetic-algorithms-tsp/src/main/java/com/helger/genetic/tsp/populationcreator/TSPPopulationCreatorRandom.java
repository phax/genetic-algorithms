...
public final class TSPPopulationCreatorRandom extends AbstractPopulationCreator
{
  ...
  {
    ValueEnforcer.isTrue (nCities >= 2, "too few cities");
    ValueEnforcer.isTrue (nPopulationSize >= 2, "too few populations!");
    ValueEnforcer.notNull (aFitnessFunction, "FitnessFunction");
  ...
  {
    final ICommonsOrderedSet <IChromosome> cs = new CommonsLinkedHashSet <> ();
    ...
    // Create a generation 0 object and add them in the provided order
    final IMutablePopulation ret = createEmptyPopulation ();
    ret.addChromosomes (cs);
    return ret;
  }
}
